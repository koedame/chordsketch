 ...
 *   - packages/playground/src/playground.css  (--cs-* prefix, :root)
 *
...
const TARGETS = [
  {
    ...
    selectors: [
      ...
      ".radio", ".switch",
      // Chrome & layout (ADR-0061). The rules themselves are generated by
      // scripts/build-chrome-css.mjs; they need the token block scoped to
      // their roots for the same reason the primitives do.
      ".topnav", ".sidenav", ".pane", ".pane-head", ".pane-body", ".stack",
    ],
  },
  {
  ...
  {
    // The playground is an application, not a library: it declares the tokens
    // at its own `:root` because its chrome selectors sit outside every
    // package's scoped root. Prefixed so the names match the ones the
    // packages' stylesheets bring to the same page.
    path: "packages/playground/src/playground.css",
    prefix: true,
    selectors: [":root"],
  },
];
