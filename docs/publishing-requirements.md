...
Generated by `post-release.yml` (`PKGBUILD` and a hand-written `.SRCINFO`)
and pushed to `aur.archlinux.org/chordsketch`.

| Condition | Source | Checked by |
|---|---|---|
| `pkgname` and `pkgver` follow the PKGBUILD rules | [PKGBUILD](https://wiki.archlinux.org/title/PKGBUILD) | `aur_problems` |
| `.SRCINFO` equals `makepkg --printsrcinfo` | [AUR submission guidelines](https://wiki.archlinux.org/title/AUR_submission_guidelines) | `aur_problems`, in an `archlinux:base-devel` container |
| `namcap PKGBUILD` reports nothing | [AUR submission guidelines](https://wiki.archlinux.org/title/AUR_submission_guidelines) | `aur_problems` |
| The source is a release asset with that asset's checksum | — | `aur_problems` |
| A package of prebuilt binaries is named `-bin` when a source build exists | [AUR submission guidelines](https://wiki.archlinux.org/title/AUR_submission_guidelines) | **not checked** — the package is published as `chordsketch`; renaming an existing AUR package is a decision for the maintainer |
...
Whether to do the Flathub submission properly or drop the dormant job is a
