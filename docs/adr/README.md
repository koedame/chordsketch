...
| [0069](0069-crates-io-and-npm-publish-from-ci-with-trusted-publishing.md) | crates.io and every npm package publish from one `workflow_dispatch`-only workflow, `publish-registries.yml`, through trusted publishing (OIDC) instead of from the maintainer's machine: its `check` mode proves each registry will mint a token and dry-runs every pending package before the tag, its `publish` mode replaces `release.py`'s local publish after CI converges, and token publishing is closed once it has worked; npm's classic-token revocation and trusted publishing remove both reasons ADR-0008 kept npm local, while new packages still need one manual publish because neither registry attaches a trusted publisher to a name that does not exist (supersedes ADR-0008 once accepted) | Proposed (2026-09-14) |
