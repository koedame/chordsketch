...
| [0034](0034-lenient-key-input-canonical-render.md) | Lenient `{key}` input, canonical render: accept the common human key spellings (`G minor`, `G m`, `Gminor`, `G major`, …) and normalise them to `Gm` / `G` / `C dorian` on every render surface while leaving the editor source untouched; only non-keys (`G7`, `H`) still warn (supersedes ADR-0033's strict-input clause) | Accepted (2026-06-17) |
