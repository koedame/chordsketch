...
| [0046](0046-linux-release-binaries-target-an-old-glibc.md) | Every `*-unknown-linux-*` release target is built through `cross` (an old sysroot) and the gnu archives declare a glibc 2.18 floor, enforced on PRs against the release matrix and at tag time against the built binaries; bottling Homebrew, swapping in musl, and documenting a 2.39 floor are all rejected | Accepted (2026-09-02) |
