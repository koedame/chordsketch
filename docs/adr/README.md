...
| [0049](0049-chocolatey-rollup-asserts-acceptance-not-approval.md) | The post-release rollup's Chocolatey check asserts that the community repository accepted the version, not that a moderator approved it (the v2 OData entity endpoint answers 200 for a queued version and 404 for one that never landed, so a 403-blocked push is the only red) | Accepted (2026-09-01) |
