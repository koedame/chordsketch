...
|---|---|---|---|
| `chordsketch-chordpro` | `crates/chordpro` | lib | *none* (zero external deps). AST + parser + transforms; ships a hand-rolled, zero-dep JSON serialiser at `src/json.rs` ([ADR-0017](docs/adr/0017-react-renders-from-ast.md), #2475) so wasm consumers can drive the React AST → JSX walker without an HTML round-trip. |
| `chordsketch-ireal` | `crates/ireal` | lib | *none* (zero external deps). iReal Pro AST + zero-dep JSON debug serializer / parser (#2055); foundation for the iReal Pro feature set tracked under #2050. AST covers the full URL grammar — `(altchord)` parens (`Chord::alternate`), `n` no-chord (`Bar::no_chord`), `Kcl` / `x` / `r` simile (`Bar::repeat_previous`), `<text>` captions (`Bar::text_comment`), and the `irealbook://` 6-field URL shape (`Title=Composer=Style=Key=TimeSig=Music`) alongside the canonical 7..=9-field `irealb://` shape. `S` / `Q` / `<D.C.>` / `<D.S.>` / `<Fine>` markers attach to the bar in which they appear (anchored macro-prefix detection guards against substring false-positives like `refine` / `define`). |
