...
|---|---|---|---|
| `chordsketch-chordpro` | `crates/chordpro` | lib | *none* (zero external deps). AST + parser + transforms; ships a hand-rolled, zero-dep JSON serialiser at `src/json.rs` ([ADR-0017](docs/adr/0017-react-renders-from-ast.md), #2475) so wasm consumers can drive the React AST → JSX walker without an HTML round-trip. The `transpose` module exposes the song-wide canonical-spelling API (`transpose_chord_with_style`, `transposed_key_prefers_flat`, `canonical_transposed_key`, `canonical_key_spelling`, `key_prefers_flat`) every renderer routes through to keep chord lines on one side of the circle of fifths (A# → Bb, D# → Eb, etc. when the song lands on a flat-side key). The `key` module is the single lenient `{key}` parser (`parse_key`, [ADR-0033](docs/adr/0033-canonical-key-directive-notation.md)/[ADR-0034](docs/adr/0034-lenient-key-input-canonical-render.md)) every key-consuming subsystem (transpose re-spelling, key-signature glyph, scale/tonic audition, displayed key) routes through; its canonical rendered form is the **spelled-out** `G major` / `G minor` / `C dorian` ([ADR-0035](docs/adr/0035-spelled-out-canonical-key-notation.md)), built via the shared `key::quality_word` so every surface agrees. It also exports `effective_transpose(file, cli, capo)` ([ADR-0023](docs/adr/0023-capo-transposes-displayed-chords.md)) — the single-source composition helper every renderer + the wasm parse path routes through so the `file + cli - capo` rule cannot drift between surfaces. The `typography` module exposes `unicode_accidentals` and `tempo_marking_for` shared by all three Rust renderers + the React JSX walker sister-site; per [`.claude/rules/chord-name-typography.md`](.claude/rules/chord-name-typography.md) every chord-name display surface — inline chord names, `{key}` directives, **and chord-diagram titles** — typesets accidentals through `unicode_accidentals`. Chord-diagram titles route through `chord_diagram::decompose_diagram_title`, which returns display-ready (already-typeset) base + tension strings consumed by the three SVG title sites and the PDF `PdfTitleLayout`; the ASCII (`render_ascii`) and render-text keyboard branches apply `unicode_accidentals` to the title directly. The `grid` module exposes the structured `{start_of_grid}` tokeniser (`tokenize_grid_line`, `classify_grid_row`, `GridShape::parse`, `extract_grid_label`) — sister-site to `tokenizeGridLine` / `parseGridShape` / `extractGridLabel` in `packages/react/src/chordpro-jsx.tsx`. Renderers route grid body content through this module to surface barlines, volta brackets, `%` / `%%` measure-repeats, cell-internal `~` multi-chord, strum rows (`|s ...`), row labels (`A` / `Coda`), and trailing comments uniformly across all four surfaces. The `voicings` module's `lookup_diagram` / `lookup_keyboard_voicing` (the single entry point every renderer + binding routes chord-diagram lookups through) consult song `{define}` directives → curated voicing tables → an **algorithmic voicing synthesiser** (`src/voicing_synth.rs`) that derives a playable diagram from a chord's pitch-class content (`chord::chord_tones`, exported as `ChordTones`). The synthesiser guarantees every chord type the editor's structured chord-type controls (triad × seventh × tensions, [ADR-0037](docs/adr/0037-explicit-chord-extension-notation.md)) can produce yields a diagram on guitar / ukulele / charango / keyboard with no per-type voicing data — coverage is kept at 100% by [`.claude/rules/chord-diagram-coverage.md`](.claude/rules/chord-diagram-coverage.md), whose sister list is the generated `enumerateEditorSuffixes()` ↔ `PALETTE_SUFFIXES`. The same module exports `diagram_pitches` (#2736) — the audio companion that runs that lookup chain and returns the MIDI notes the drawn diagram **sounds** (per-string for fretted instruments via `DiagramData::voiced_pitches` against the `voicing_synth` tunings, highlighted keys for keyboard) so a chord diagram can be auditioned as exactly the shape it depicts, distinct from `chord::chord_pitches`'s name-derived block voicing; it is exposed across the wasm / napi / ffi bindings as `diagramPitches` / `diagram_pitches`. The `chord` module also exports `suggest_canonical_chord`, and `render_result::validate_ambiguous_chords` warns across all four surfaces ([ADR-0037](docs/adr/0037-explicit-chord-extension-notation.md), #2705): ambiguous bare-stack notation (`G13`, `C(9)`) still parses and renders unchanged but is flagged toward the explicit spelling (`G7(9,11,13)` / `G7(13)`, `Cadd9` / `C7(9)`), sister to the `{key}` `validate_keys` diagnostic. |
...
| `chordsketch-import-gp` | `crates/import-gp` | lib | `chordsketch-chordpro`, `chordsketch-convert` (zero other external deps). Guitar Pro 5 (`.gp5`, versions 5.00 and 5.10) → ChordPro importer for `chordsketch convert song.gp5` / `--from gp5` / `--track N`. Guitar Pro 3, 4, 6 and 7 files (`.gp3`, `.gp4`, `.gpx`, `.gp`) are recognised and rejected with `GpError::UnsupportedVersion` rather than falling through to a UTF-8 decode error. Reads chord names from one track's chord diagrams (raised to sounding pitch next to `{capo}` when the track has a capo, per ADR-0023) and lyrics split into syllables the way Guitar Pro lays them over the notes, laid onto a shared (measure, tick) timeline so the chords and lyrics may come from different tracks. A diagram whose name is not a chord is respelled from its stored root and chord type (`WarningKind::Approximated`) or dropped (`WarningKind::LossyDrop`) via `chordsketch-convert`'s `ConversionOutput`. Every read in the binary parser is bounds-checked and every in-file count is checked against the bytes remaining before it is used to allocate, so parsing a truncated or corrupted file is linear in the input and returns `GpError::Truncated` / `GpError::InvalidData` instead of panicking; input is capped at `MAX_INPUT_BYTES` (16 MiB). |
| `chordsketch` (CLI) | `crates/cli` | bin | `chordsketch-chordpro`, `chordsketch-ireal`, `chordsketch-render-text`, `chordsketch-render-html`, `chordsketch-render-pdf`, `chordsketch-render-ireal`, `chordsketch-convert-musicxml`, `chordsketch-import-gp`, `chordsketch-mcp`. CLI auto-detects ChordPro vs `irealb://` input (#2335). `chordsketch mcp` serves the Model Context Protocol over stdio (ADR-0063). |
| `chordsketch-lsp` | `crates/lsp` | bin | `chordsketch-chordpro`, `tower-lsp`, `tokio` |
| `chordsketch-mcp` | `crates/mcp` | lib | `chordsketch-chordpro`, `chordsketch-render-text`, `chordsketch-render-html`, `rmcp` (the official Rust MCP SDK), `tokio`, `serde` / `serde_json` / `schemars`. Model Context Protocol server exposing six ChordPro tools (`render_chordpro` / `parse_chordpro` / `validate_chordpro` / `format_chordpro` / `chord_diagram_svg` / `list_directives`) over stdio. The executable entry point is the CLI's `chordsketch mcp` subcommand, not a second binary ([ADR-0063](docs/adr/0063-mcp-server-is-a-cli-subcommand.md)) — the packaging channels install exactly one binary, so riding the CLI is what makes the server reachable through all of them. `src/ops.rs` holds the ChordPro operations as plain functions with no MCP types; `src/server.rs` is the protocol adapter over it. Deliberately **not** a fourth member of the FFI / WASM / NAPI sister-site group in `.claude/rules/fix-propagation.md`: it consumes the workspace crates the way the CLI does rather than re-exporting an API surface, and its tool set is a cut for AI callers (no PDF, no iReal Pro, no config files), not a projection of the binding surface. No filesystem and no network access — every tool takes ChordPro source as a string, capped at the parser's own 10 MiB limit. |
...
| `chordsketch-desktop` | `apps/desktop/src-tauri` | bin | `chordsketch-chordpro`, `tauri` (v2). Rust shell for the desktop app; loads `apps/desktop/dist/` (Vite build of a React app that mounts `@chordsketch/react`'s `<ChordProEditor>` / `<IrealProEditor>` against `@chordsketch/wasm`) inside the WebView. A `desktopBridge` singleton routes Tauri menus / Open-Save dialogs / updater events into the React layer (#2527). Excluded from default workspace operations via `default-members` because its transitive deps need webkit2gtk / WebView2. CI's `desktop-smoke` job in `ci.yml` covers fast Linux regression catching on every PR; `desktop-build.yml` runs the full 4-cell matrix (macOS x86_64 + aarch64, Windows, Linux) and uploads unsigned installer bundles as workflow artefacts (#2077); `desktop-release.yml` fires on `desktop-v*` tags and publishes a GitHub Release with bundles + `SHA256SUMS` (#2078), pushes `Casks/chordsketch.rb` to `koedame/homebrew-tap` so `brew install --cask koedame/tap/chordsketch` picks up the new DMG (#2079), and publishes `latest.json` for the Tauri updater plugin to drive in-app auto-updates (#2076; keys managed per ADR-0005). Apple Developer ID signing / notarization (#2075) still layers on top for Gatekeeper. |
| `chordsketch-preview-handler` | `apps/desktop/preview-handler` | cdylib | `chordsketch-chordpro`, `chordsketch-render-html`; on Windows also `windows`, `windows-core`, `raw-window-handle`, `wry`. Windows Explorer preview handler (#2289) — an in-process COM server (`IPreviewHandler` + `IInitializeWithStream` + `IObjectWithSite` + `IOleWindow`) that Explorer loads into its `prevhost.exe` surrogate to draw `.cho` / `.chopro` / `.crd` / `.chordpro` files in the preview pane. Ships inside the desktop installer as `chordsketch_preview_handler.dll`; both installers register it declaratively at their own scope (`HKMU` in the WiX fragment, `SHCTX` in the NSIS hooks) rather than through a self-registering `DllRegisterServer` ([ADR-0050](docs/adr/0050-windows-preview-handler-is-installer-registered.md)). The pane is a WebView2 control (via `wry`, the same embedding layer Tauri uses) showing `chordsketch-render-html`'s fragment output wrapped in an envelope that carries a `default-src 'none'` Content Security Policy. Excluded from `default-members` alongside `chordsketch-desktop` for the same WebView2 reason; everything outside the platform-independent `document` module is `cfg(windows)`-gated, so the `--workspace` clippy / test / doc jobs cover it on every runner and the Windows cell of `ci.yml`'s test matrix compiles the COM server on every PR. |
...
| `@chordsketch/react` | `packages/react` | React component library. v0.1.0 — first publishable release (#2473); v0.2.0 — iReal Pro surface reaches `@chordsketch/ui-irealb-editor` parity (#2505); v0.3.0 — three-tier component layout + hard symbol renames per [ADR-0022](docs/adr/0022-react-as-canonical-preview-surface.md) (#2527); v0.4.0 — the ChordPro chord editor lifts to a full-width footer spanning the editor + preview, driven by the editor caret (caret-on-chord auto-selects and the edit-only footer retypes / moves / removes it; idle shows a hint; the preview caret-marker is suppressed while a chord is selected), via the new `useChordEditor` hook + `<ChordSheet>`'s controlled-selection mode (`chordSelection` / `onChordSelectionChange`); the in-pane `<ChordInspector>` stays for standalone `<ChordSheet>` use (#2644, #2646, #2648). `<ChordInspector>`'s chord-type controls are three orthogonal groups — triad quality / seventh / tensions — that compose only explicit, unambiguous suffixes ([ADR-0037](docs/adr/0037-explicit-chord-extension-notation.md), #2705): never the ambiguous bare stack `G13` (the editor writes `G7(13)` / `G7(9,11,13)`) nor `C(9)` (it writes `Cadd9` / `C7(9)`); `composeChordSuffix` / `decomposeChordSuffix` are the compose/normalise pair. ChordPro surface (`<PdfExport>`+`usePdfExport`, `<ChordSheet>`+`useChordRender`, `<ChordTextarea>`+`useDebounced`, `<Transpose>`+`useTranspose`, `<ChordDiagram>`+`useChordDiagram`, `<ChordSourceArea>`, `<SplitLayout>`, `<RendererPreview>`, `<ChordProPreview>`, `<ChordProEditor>`, `<ChordInspector>`+`useChordEditor`) + iReal Pro surface (`<IrealBarGrid>`+`useIrealParse`+`useIrealSerialize`, `<IrealPreview>`+`useIrealRender`, `<IrealProEditor>`, `Ireal*` AST type re-exports). v0.3.0 renamed the legacy `<Playground>` / `<ChordEditor>` / `<SourceEditor>` / `<IrealEditor>` / `<IrealPlayground>` names with no deprecated aliases — the `Editor` suffix now denotes Tier 3 composed editors (`<ChordProEditor>`, `<IrealProEditor>`) while Tier 1 atoms use widget-type names (`<ChordTextarea>`, `<ChordSourceArea>`, `<IrealBarGrid>`). The `<ChordSheet format="html">` branch renders AST → JSX directly via the `chordpro-jsx` walker ([ADR-0017](docs/adr/0017-react-renders-from-ast.md), #2475) — `<RendererPreview format="html">` no longer wraps the output in an iframe. The `format="text"` branch keeps the wasm `render_text` path; `format="pdf"` stays a download action via `<PdfExport>`. The walker's inline `{tempo}` chip is rendered by an interactive `<MetronomeButton>` (backed by the `useMetronome` Web Audio hook) — the whole chip is the click target (clicking anywhere on the pill ticks audibly at the directive's BPM), the cursor becomes a speaker on hover, and while playing the chip's frame colour pulses once per beat; it degrades to a static, non-interactive chip under SSR / browsers without `AudioContext`. The walker's inline `{key}` chip is rendered by an interactive `<KeySignatureButton>` (backed by the `useKeyAudio` Web Audio hook, #2658) — clicking anywhere on the pill auditions the key by ear: the movable-do scale "do re mi fa sol la ti do" then the tonic triad strummed, major or minor per the key, sourced from the `keyScalePitches` / `keyTonicTriad` core exports; when a transpose is active it auditions the sounding key, and it degrades to the same static chip under SSR / no `AudioContext`. `unicodeAccidentals` lives in the leaf `music-glyphs` module (re-exported from `chordpro-jsx`) so the button and walker share it without a circular import; the shared lazy-wasm loader (`usePitchModule`) and oscillator stop helper (`stopVoices`) back both `useChordAudio` and `useKeyAudio`. Clicking a chord **diagram** (the end-of-song grid, inline / hover cells, or standalone `<ChordDiagram chordAudio>`) auditions the diagram's **own voicing** — the per-string fretted pitches or the keyboard's highlighted keys — not the name-based block voicing (#2736): `useChordDiagramPitches` resolves those pitches from the core `diagramPitches` export (the same lookup that draws the SVG, so audio and the drawn shape cannot drift) and `useChordAudio.playPitches` (wired through `ChordAudioConfig.playPitches`) strums them; it degrades to the chord-name block voicing on `@chordsketch/wasm` bundles predating the export. The iReal Pro surface is a native React implementation per [ADR-0020](docs/adr/0020-ireal-pro-react-surface.md): v0.2.0 ships the interactive bar grid (ARIA `role="grid"` + roving tabindex + keyboard navigation), structural editing (section / bar add / rename / delete / move), and popover-based per-bar chord editing (`<IrealBarPopover>` with focus trap, chord-row editor, N-th ending input, symbol picker). Dual ESM + CJS build via tsup; React 18+ peer dep; CSS at `@chordsketch/react/styles.css`. published by dispatching `publish-registries.yml` per [ADR-0069](docs/adr/0069-crates-io-and-npm-publish-from-ci-with-trusted-publishing.md). |
| `@chordsketch/react-ui` | `packages/react-ui` | npm package — **wasm-free** React design-system primitives (`<Button>`, `<Card>`, `<Badge>` / `<Pill>`, and form controls `<Input>` / `<Textarea>` / `<Select>` / `<Checkbox>` / `<Radio>` / `<Switch>` / `<Segmented>` / `<Field>`) binding the canonical design-system class vocabulary from `design-system/DESIGN.md` §6 / `design-system/preview/components-*.html`. Per [ADR-0029](docs/adr/0029-react-ui-primitives-package.md): no `@chordsketch/wasm*` anywhere in its dependency graph (enforced by `tests/no-wasm-dep.test.ts`); `@chordsketch/react` stays domain-only and does NOT re-export it — when a domain component composes a primitive it declares `@chordsketch/react-ui` as a `peerDependency` to keep a single instance. CSS at `@chordsketch/react-ui/styles.css` (ships the `--cs-*` token layer + component rules; the design-system token names stay the source of truth). The same stylesheet also ships the design system's **chrome and layout** vocabulary as CSS with no React component — `.topnav` and its parts, `.sidenav`, `.pane` / `.pane-head` / `.pane-body`, and `.stack` + its `--cs-stack-gap` modifiers — generated out of the `design-system/` reference pages by `scripts/build-chrome-css.mjs` and held at a zero diff by the same `tokens-sync` CI check ([ADR-0061](docs/adr/0061-chrome-and-layout-css-ships-from-react-ui.md)). Dual ESM + CJS via tsup; React 18+ peer dep; published by dispatching `publish-registries.yml` per [ADR-0069](docs/adr/0069-crates-io-and-npm-publish-from-ci-with-trusted-publishing.md). The playground consumes its `<Button>` via a vite alias (`packages/playground/vite.config.ts`). |
| `@chordsketch/chordpro-lite` | `packages/chordpro-lite` | npm package — **dependency-free** ChordPro text helpers: `detectFormat` (ChordPro / `irealb://` / neither), `extractLyrics` (lyric words only, for indexing / plain-text export) and `extractPreview` (the opening chord + lyric lines, capped by `maxLines` / `maxChordsPerLine` / `maxLyricChars`). Per [ADR-0060](docs/adr/0060-wasm-free-chordpro-text-helpers-package.md): the three questions a consumer must answer **before** deciding to load the engine are pure string work, so they ship with no dependencies at all — no `@chordsketch/wasm*` anywhere in the graph (enforced by `tests/no-wasm-dep.test.ts`, same policy shape as `@chordsketch/react-ui`). The value-less directive names `detectFormat` recognises are **generated** from `BARE_DIRECTIVE_NAMES` in `crates/chordpro/src/directive_catalog.rs` by `scripts/generate-bare-directives.py`, so the sniffer cannot fall behind the parser (ADR-0028's catalog is still the single source of truth); the unfiltered `directive-catalog-sync` job in `ci.yml` fails on drift. Dual ESM + CJS via tsup; no peer deps; published by dispatching `publish-registries.yml` per [ADR-0069](docs/adr/0069-crates-io-and-npm-publish-from-ci-with-trusted-publishing.md). |
| `@chordsketch/vue` | `packages/vue` | npm package — Vue 3 (Composition API) bindings for the ChordPro surface: `<ChordSheet>` + `useChordRender`, `<ChordTextarea>` + `useDebounced`, `<ChordDiagram>` + `useChordDiagram`, `<Transpose>` + `useTranspose`, and `<PdfExport>` + `usePdfExport`. Prop names, defaults and DOM/class vocabulary match `@chordsketch/react`; the framework-shaped differences are documented in the package README (React render-prop fallbacks are slots, callbacks are `update:*` events, composables take an options object and return refs). `<ChordSheet format="html">` renders the **static-output HTML** surface (`render_html_body`) rather than the React AST → JSX walker ([ADR-0017](docs/adr/0017-react-renders-from-ast.md) makes `chordsketch-render-html` canonical for non-React consumers), and injects `render_html_css` rewritten so every rule is scoped to the component's own wrapper — so the React-only interaction props built on the walker (chord selection, drag-to-reposition, chord audio) have no Vue equivalent. Dual ESM + CJS via tsup; Vue 3.3+ peer dep; `@chordsketch/wasm-export` is an optional peer used only by `<PdfExport>`. CSS at `@chordsketch/vue/styles.css`. Published by dispatching `publish-registries.yml` per [ADR-0069](docs/adr/0069-crates-io-and-npm-publish-from-ci-with-trusted-publishing.md). |
| `@chordsketch/svelte` | `packages/svelte` | npm package — Svelte 5 (runes) bindings for the ChordPro surface: `<ChordSheet>` + `useChordRender`, `<ChordTextarea>` + `useDebounced`, `<ChordDiagram>` + `useChordDiagram`, `<Transpose>` + `useTranspose`, and `<PdfExport>` + `usePdfExport`. Prop names, defaults and DOM/class vocabulary match `@chordsketch/react`; the framework-shaped differences are documented in the package README (React render-prop fallbacks are snippets, `value` + `onChange` pairs are a single `bind:value`, and the state helpers take getters and return objects whose properties are reactive). `<ChordSheet format="html">` renders the **static-output HTML** surface (`render_html_body`) and scopes `render_html_css` to its own wrapper, exactly as `@chordsketch/vue` does and for the same reason ([ADR-0017](docs/adr/0017-react-renders-from-ast.md) makes `chordsketch-render-html` canonical for non-React consumers) — so the React-only interaction props built on the walker (chord selection, drag-to-reposition, chord audio) have no Svelte equivalent. Unlike the React / Vue packages this one is **not bundled**: `svelte-package` emits the components as preprocessed `.svelte` sources plus generated `.d.ts`, which is how a Svelte library is consumed (the host's compiler compiles them). Svelte 5.0+ peer dep; `@chordsketch/wasm-export` is an optional peer used only by `<PdfExport>`. CSS at `@chordsketch/svelte/styles.css`. Published by dispatching `publish-registries.yml` per [ADR-0069](docs/adr/0069-crates-io-and-npm-publish-from-ci-with-trusted-publishing.md). |
...
| ChordSketch (Claude Code plugin) | `packages/claude-code-plugin` | Claude Code plugin carrying the `/chordpro` skill (`skills/chordpro/SKILL.md` plus on-demand `references/`). Installed from this repository, which doubles as the plugin marketplace via `.claude-plugin/marketplace.json` at the root ([ADR-0059](docs/adr/0059-claude-code-skill-ships-as-a-marketplace-plugin.md)). The skill drives the CLI for render / transpose / validate / fmt / convert and documents the parse-tree path against the published `@chordsketch/wasm`, which the CLI has no equivalent for. Both `.claude-plugin/plugin.json` and the root marketplace entry are held at the workspace version by `scripts/check-version-consistency.py`: Claude Code caches plugins under `<marketplace>/<plugin>/<version>/` and will not re-fetch an unchanged version, so a missed bump means installed clients keep the previous skill. Distinct from `.claude/skills/`, which is development scaffolding for this repository and reaches nobody who has not cloned it. |
