...
| `@chordsketch/wasm` | `^0.6.0` (runtime dep) | Bundled as a regular dependency; hosts can override at hoist time if they want a specific minor. |
| `@chordsketch/wasm-export` | `^0.6.0` (optional peer) | Required for `<PdfExport>` / `usePdfExport`. Lazy-loaded on first export. |
