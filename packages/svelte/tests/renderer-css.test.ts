...
describe('injectCss', () => {
  ...
  test('appends the stylesheet once, however many times it is requested', () => {
    ...
    injectCss('.a { color: red }');
    const styles = document.head.querySelectorAll('style[data-chordsketch-svelte]');
  ...
  test('a different stylesheet gets its own element', () => {
    ...
    expect(document.head.querySelectorAll('style[data-chordsketch-svelte]')).toHaveLength(2);
  });
});
