...
// `import()` keeps the Svelte bundle free of the heavy WASM glue
// until a consumer actually triggers an export.
...
export interface UsePdfExportResult {
   ...
   * error is written to `error` before the promise settles, so UIs
   * that prefer state-driven rendering can ignore the rejection
   * (e.g. `exportPdf(...).catch(() => {})`) and render from `error`
   * instead.
   */
  exportPdf: (
    source: string,
...
/**
 * Rune helper that produces a `Promise<void>`-returning `exportPdf`
 * function together with reactive `loading` / `error` state. The
 * WASM module is loaded at most once per loader and reused across
 * calls.
 *
 * Unlike the render helpers this one registers no `$effect`, so it
 * can also be called outside component initialisation.
 *
 * ```ts
 * const pdf = usePdfExport();
 * // later:
 * await pdf.exportPdf(chordproSource, 'song.pdf');
 * ```
 *
 * ### Cache scope
 *
