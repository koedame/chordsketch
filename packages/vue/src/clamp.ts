 ...
 *
 * Shared between {@link useTranspose} (where `NaN` can leak in from
 * parsed numeric-input values) and the `<Transpose>` component
 * (where both operands are guaranteed finite numbers, but sharing
 * the helper guarantees any future normalisation — e.g. rounding —
