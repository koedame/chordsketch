...
/// Returns the path to one of the Guitar Pro importer's fixtures.
fn gp_fixture(name: &str) -> String {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../import-gp/tests/fixtures")
        .join(name)
        .to_string_lossy()
        .to_string()
}
...
#[test]
fn test_convert_gp5_import_by_extension() {
    Command::cargo_bin("chordsketch")
        .unwrap()
        .args(["convert", &gp_fixture("harbor-lights.gp5")])
        .assert()
        .success()
        .stdout(predicate::str::contains("{title: Harbor Lights}"))
        .stdout(predicate::str::contains("{capo: 2}"))
        .stdout(predicate::str::contains("{start_of_chorus: Chorus}"))
        .stdout(predicate::str::contains("[F#m]Lanterns on [D]the [A]water"))
        .stderr(predicate::str::contains("warning:"))
        .stderr(predicate::str::contains("spelled as \"Gmaj7\""));
}

#[test]
fn test_convert_gp5_import_from_stdin_by_header() {
    let bytes = std::fs::read(gp_fixture("amazing-grace.gp5")).unwrap();
    Command::cargo_bin("chordsketch")
        .unwrap()
        .args(["convert", "-"])
        .write_stdin(bytes)
        .assert()
        .success()
        .stdout(predicate::str::contains("A[G]mazing [G7]grace"))
        .stdout(predicate::str::contains("{key: G}"));
}

#[test]
fn test_convert_gp5_import_forced() {
    let bytes = std::fs::read(gp_fixture("greensleeves.gp5")).unwrap();
    let mut file = NamedTempFile::with_suffix(".bin").unwrap();
    file.write_all(&bytes).unwrap();
    Command::cargo_bin("chordsketch")
        .unwrap()
        .args(["convert", "--from", "gp5", &file.path().to_string_lossy()])
        .assert()
        .success()
        .stdout(predicate::str::contains("{time: 6/8}"))
        .stdout(predicate::str::contains("[Am]Alas my love"));
}

#[test]
fn test_convert_gp5_track_selects_the_chord_track() {
    Command::cargo_bin("chordsketch")
        .unwrap()
        .args(["convert", &gp_fixture("harbor-lights.gp5"), "--track", "3"])
        .assert()
        .success()
        .stdout(predicate::str::contains("Lanterns on the water"))
        .stdout(predicate::str::contains("[F#m]").not())
        .stderr(predicate::str::contains(
            "track 3 (Bass) has no chord diagrams",
        ));
}

#[test]
fn test_convert_gp5_track_out_of_range() {
    Command::cargo_bin("chordsketch")
        .unwrap()
        .args(["convert", &gp_fixture("harbor-lights.gp5"), "--track", "4"])
        .assert()
        .failure()
        .stderr(predicate::str::contains(
            "track 4 does not exist; the file has 3 track(s): 1: Vocal Melody, 2: Rhythm Guitar, 3: Bass",
        ));
}

#[test]
fn test_convert_track_rejected_for_non_guitar_pro_input() {
    Command::cargo_bin("chordsketch")
        .unwrap()
        .args(["convert", &fixture("simple.cho"), "--track", "1"])
        .assert()
        .failure()
        .stderr(predicate::str::contains(
            "--track applies only to Guitar Pro input",
        ));
    Command::cargo_bin("chordsketch")
        .unwrap()
        .args([
...
#[test]
fn test_convert_unsupported_guitar_pro_version_is_named() {
    // A Guitar Pro 7 file is a ZIP archive, not a GP5 file.
    let mut file = NamedTempFile::with_suffix(".gp").unwrap();
    file.write_all(b"PK\x03\x04not-a-gp5-file").unwrap();
    Command::cargo_bin("chordsketch")
        .unwrap()
        .args(["convert", &file.path().to_string_lossy()])
        .assert()
        .failure()
        .stderr(predicate::str::contains("not a Guitar Pro 5 file"));
}
