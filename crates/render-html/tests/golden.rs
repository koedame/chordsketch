//! Golden tests for the HTML renderer.
//!
//! Discovers every subdirectory under `tests/fixtures/` that contains an
//! `input.cho`, runs [`chordsketch_render_html::render`], and compares the
//! output against `expected.html`.
//!
//! Each fixture is named after its render-text counterpart so sister-site
//! parity (see `.claude/rules/renderer-parity.md`) can be verified by
//! cross-referencing directory listings.
//!
//! Regenerate snapshots when the HTML output changes intentionally:
//!
//! ```sh
//! UPDATE_GOLDEN=1 cargo test -p chordsketch-render-html --test golden
//! ```
...
fn run_golden_test(fixture_dir: &Path) {
    ...
    let expected = fs::read_to_string(&expected_path)
        .unwrap_or_else(|e| {
            panic!(
                "[{name}] cannot read {} (run `UPDATE_GOLDEN=1 cargo test -p chordsketch-render-html --test golden` to create it): {e}",
