...
.music-glyph--metronome__pendulum { transform-origin: 9px 19px; animation: cs-metronome-swing var(--cs-metronome-period, 1s) cubic-bezier(0.7, 0, 0.3, 1) infinite alternate; }
@keyframes cs-metronome-swing { 0%, 12% { transform: rotate(-28deg); } 88%, 100% { transform: rotate(28deg); } }
