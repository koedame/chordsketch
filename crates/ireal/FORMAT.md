...
| `<...>` | Comment / text mark. **Exact-match** classification (#2427) against the eleven player-recognised phrases from <https://www.irealpro.com/ireal-pro-custom-chord-chart-protocol> plus the bare legacy forms: `D.C.` / `D.C. al Coda` / `D.C. al Fine` / `D.C. al 1st End.` / `D.C. al 2nd End.` / `D.C. al 3rd End.` → `MusicalSymbol::DaCapo(JumpTarget)`; the same set with `D.S.` prefix → `DalSegno(JumpTarget)`; `Fine` → `Fine`; `Break` → `MusicalSymbol::Break` (drum-silence directive — the player stops drums "with a shot on one" until the next double bar — distinct from the bar-level `n` token which silences only chordal instruments; renderer paints the literal word "Break" as italic staff text; spec: <https://technimo.helpshift.com/hc/en/3-ireal-pro/faq/101-n-c-symbol-break/?p=all>). The ending suffix tolerates three synonym spellings — `End.` (spec), `End`, `Ending` — but the ordinal must agree (`1st`/`2nd`/`3rd`/`4th`); a non-spec suffix (`1rd`, `2st`) falls through. Case-insensitive, whitespace-tolerant. Anything outside the exact match table (`<13 measure lead break>`, `<D.C. on Cue>`, `<solo break>`) appends a [`StaffText`] entry to `Bar::staff_texts` instead (#2426), leaving `Bar::symbol = None`. The parser's "either symbol OR staff_texts, never both" invariant prevents the round-trip duplication of bar markers that occurred before #2427. The spec's two structured staff-text shapes — `<*XYtext>` (vertical position prefix, `XY` ∈ `00..=74`) and `<Nx>` (repeat-count override for the enclosing `{ ... }` block) — are recognised at parse time and produce `StaffText::Text { vertical_position: Some(_), .. }` / `StaffText::RepeatCount(_)` respectively; everything else falls through to `StaffText::Text { vertical_position: None, .. }` so any caption survives the round trip verbatim. Multiple `<...>` tokens on one bar produce multiple `StaffText` entries in source order. |
...
| `p` | Pause slash — emits a [`BarChord`] with `kind = BarChordKind::SlashRepeat` whose `chord` field carries a snapshot of the preceding chord. The renderer paints a single `/` glyph at that beat slot instead of chord typography; the SlashRepeat chord still serializes back to `p` on the URL round trip. An orphan `p` with no preceding chord (malformed URL) is dropped silently. Inside `(...)` alternate-chord parens the slash is also dropped — alternates are an annotation on the primary chord, not a beat slot the slash could legitimately occupy. |
| `<a+b(+c)*>` | Compound-time beat grouping (#2449) — a staff-text token whose body is a `digit+digit(+digit)*` sequence (`2+3`, `3+2`, `3+2+2`, …). When the parsed grouping's sum equals the active time signature's numerator the parser stores it as [`BeatGrouping`] on the bar's `beat_grouping_override` field. Per the spec, the override "remains until the opposite is used", so the parser tracks a running grouping state and stamps it onto every subsequent bar until a new override appears or the time signature itself changes (a meter change resets the running state — an explicit re-assert is required under the new meter). Sum-mismatched groupings (`<3+3>` under 5/4) and malformed shapes (`<2++3>`, `<+3>`, `<2+>`) fall through to `Bar::staff_texts` (#2426) so the original token survives the round trip. The URL serializer emits `<a+b>` only at change points (start of an override, transitions between overrides, transitions back to default) so round-trip output matches the spec's one-token-per-change convention. |
