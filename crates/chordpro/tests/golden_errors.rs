...
//! [`chordsketch_chordpro::parse`], and verifies that parsing fails with an error
//! whose `Display` output matches the content of `expected_error.txt`.
//!
//! # Adding a new error golden test
//!
//! 1. Create a new subdirectory under `crates/chordpro/tests/error_fixtures/` with
//!    a descriptive kebab-case name (e.g., `unclosed-chord`).
//! 2. Add an `input.cho` file containing the malformed ChordPro source.
//! 3. Run `UPDATE_GOLDEN=1 cargo test -p chordsketch-chordpro --test golden_errors`
...
//!
//! ```sh
//! UPDATE_GOLDEN=1 cargo test -p chordsketch-chordpro --test golden_errors
//! ```
...
fn run_error_golden_test(fixture_dir: &Path) {
    ...
    let err = chordsketch_chordpro::parse(&input).expect_err(&format!(
    ...
    let expected = fs::read_to_string(&expected_path)
        .unwrap_or_else(|e| {
            panic!(
                "[{name}] cannot read {} (run `UPDATE_GOLDEN=1 cargo test -p chordsketch-chordpro --test golden_errors` to create it): {e}",
