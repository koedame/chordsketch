...
fn multi_song_golden_test() {
    ...
    let songs = chordsketch_chordpro::parse_multi(&input)
        .unwrap_or_else(|e| panic!("parse_multi error: {e}"));
    ...
    let expected = fs::read_to_string(&expected_path)
        .unwrap_or_else(|e| {
            panic!(
                "cannot read {} (run `UPDATE_GOLDEN=1 cargo test -p chordsketch-chordpro --test golden_multi_song` to create it): {e}",
    ...
    assert_eq!(
        expected, actual,
        "multi-song golden test failed! Run `UPDATE_GOLDEN=1 cargo test -p chordsketch-chordpro --test golden_multi_song` to update."
    );
}
...
fn new_song_at_start_golden_test() {
    ...
    assert_eq!(
        expected, actual,
        "new-song-at-start golden test failed! Run `UPDATE_GOLDEN=1 cargo test -p chordsketch-chordpro --test golden_multi_song` to update."
    );
}
