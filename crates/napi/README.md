...
| `chordDiagramSvgWithOrientation(chord, instrument, orientation?, stringOrder?)` | `string \| null` | Orientation-aware variant. `orientation`: `"vertical"` (default) or `"horizontal"` (nut on the left, Japanese tablature convention). `stringOrder`: `"reader"` (default — high pitch on top, matches tablature stave order) or `"player"` (low pitch on top — what a right-handed player sees). Unrecognised strings fall back to the defaults. |
| `chordDiagramSvgWithDefinesOrientation(chord, instrument, defines, orientation?, stringOrder?)` | `string \| null` | Combined surface — accepts both song-level `{define}` voicings and the orientation knob. |
